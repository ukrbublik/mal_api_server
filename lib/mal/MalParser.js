...
class MalParser extends MalDataProvider {
  ...
  init(options = {}) {
    ...
    this.queue = new Queue(this.options.parserQueueSizeConcurrent, 
      Infinity);
  }
  ...
  loadUrl(url) {
    if (this.options.endpoint !== null) {
      //todo?
      //for simple relay/proxy
      //url = this.options.endpoint + '?' + querystring.stringify({ url: url });
    }
      ...
      .catch((err) => {
        if (err instanceof MalError && err.body) {
          ...
          if ($('#content .badresult').length) {
            ...
            || err.response.statusCode == 404 && this.isTrueMalSite($, err.body, err.response)) {
            err.errCode = 2;
          }
          delete err.body;
          delete err.response;
  ...
  /**
   * If using relay and getting 404, we must be sure that it's from original MAL site
   */
  isTrueMalSite($, body, response) {
  ...
  getUserList({login, altJson = false}) {
    ...
    if (!altJson) {
      ...
      return this.loadXml(url).then((res) => {
        let animeList = res.myanimelist.anime;
        let maxUpdated = 0;
        if (animeList)
        ...
        return {
          ...
          listUpdated: maxUpdated ? new Date(maxUpdated) : null,
        };
      });
    } else {
        ...
        return this.loadJson(url).then((res) => {
          return new Promise((resolve, reject) => {
            if (res instanceof Array) {
              ...
              let err = "JSON result object is not array";
              if (res.errors && res.errors instanceof Array && res.errors.length) {
                err = res.errors[0].message; //"invalid request"
              }
              reject(err);
            }
          });
  ...
  getAnimeInfo({animeId}) {
    ...
    return this.loadHtml(url).then(($) => {
      ...
      if (err instanceof MalError && err.statusCode == 404 && err.errCode == 2) {
